...
model2endpoint = {
    "Qwen2.5-VL-3B-Instruct": "u1x6o4zwi182",
}
...
def ask_category(
    ...
    try:
        ...
        base_url = "https://api.friendli.ai/dedicated/v1/chat/completions"
        
        payload = {
            "messages": messages,
            "model": model2endpoint[model],
            "max_tokens": max_tokens,
            "temperature": temperature,
        }
        print(messages)
        headers = {
            "Authorization": f"Bearer {os.getenv("FRIENDLI_TOKEN")}",
...
def main(args: argparse.Namespace):
    ...
    assert "FRIENDLI_TOKEN" in os.environ
    ...
    for idx, (episode_history, item) in enumerate(tqdm.tqdm(dataset.items())):
        ...
        if "the category is" in lower_output:
            ...
            words = [re.sub(r"\W+", "", word) for word in lower_output.split()]
